...
public:
    ...
    bool isSideCollision(int x, int y, std::string* fig, int strSize)
    {
        for (int figY = 0; figY < strSize; figY++)
        {
            for (int figX = 0; figX < strSize; figX++)
    ...
    void BigBaDaBoom(int start)
    {
        ...
        {
            for (int fieX = 1; fieX < 11; fieX++)
            {
                if ((fullField[fieY - 1][fieX] == ' ') && (fieY == 19))
                    fullField[fieY][fieX] = '_';
                else
...
private:
    static const int figureSize = 4;
    std::string* figure = new std::string[figureSize];   //  4 - минимум, чтобы влезла палка
    int positionX;
    int positionY;
    figureName name;
...
public:
    Figure(figureName fName, int x = 4, int y = 1) : name(fName), positionX(x), positionY(y)
    {
        switch (fName)
        {
...
int main()
{
    ...
    {      
        ...
        while (!(tetris.isFloorCollision(x, y, figure.getFigure(), figure.getSize()) || (tetris.isDownBrickCollision(x, y, figure.getFigure(), figure.getSize()))))
        {
            ...
            auto timeCheckEndMove = std::chrono::system_clock::now();
            if (GetAsyncKeyState(65) && (std::chrono::duration_cast<std::chrono::milliseconds>(abs(timeCheckEndRot - timeCheckStartRot)).count() > timeForRotate))
            {
                figure.RotateL(figure.getFigureName());
                timeCheckStartRot = std::chrono::system_clock::now();
            }
            if (GetAsyncKeyState(68) && (std::chrono::duration_cast<std::chrono::milliseconds>(abs(timeCheckEndRot - timeCheckStartRot)).count() > timeForRotate))
            {
                figure.RotateR(figure.getFigureName());
                timeCheckStartRot = std::chrono::system_clock::now();
            }
            timeCheckEndMove = std::chrono::system_clock::now();
            if (GetAsyncKeyState(VK_LEFT) && (std::chrono::duration_cast<std::chrono::milliseconds>(abs(timeCheckEndMove - timeCheckStartMove)).count() > timeForMove))
            {
                if(!tetris.isWallCollision(x-1, y, figure.getFigure(), figure.getSize()))
                    figure.setPosition(x--, y);
                timeCheckStartMove = std::chrono::system_clock::now();
            }
            if (GetAsyncKeyState(VK_RIGHT) && (std::chrono::duration_cast<std::chrono::milliseconds>(abs(timeCheckEndMove - timeCheckStartMove)).count() > timeForMove))
            {
                if (!tetris.isWallCollision(x+1, y, figure.getFigure(), figure.getSize()))
                    figure.setPosition(x++, y);
                timeCheckStartMove = std::chrono::system_clock::now();
